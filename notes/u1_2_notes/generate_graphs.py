...
# UNIT 1 SECTION 1.2 NOTES GRAPH GENERATION BLOCKS
...
plt.close(fig)
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_piecewise_graph(ax, [{'expr': lambda x: 0.4*(x-(-2))+(1), 'deriv': lambda x: np.full_like(x,0.4,dtype=float), 'domain': (-10,-2), 'include_left': True, 'include_right': False, 'arrow_left': True, 'arrow_right': False},
{'expr': lambda x: -0.2*(x-(-2))+(4), 'deriv': lambda x: np.full_like(x,-0.2,dtype=float), 'domain': (-2,10), 'include_left': True, 'include_right': True, 'arrow_left': False, 'arrow_right': True}], title='')
save_graph(fig, 'u1_s2_notes_one_sided_compare.png')
plt.close(fig)
