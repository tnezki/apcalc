...
_save(fig, 'blank_standard.png')

def g_left(x):
    x=np.asarray(x); return np.where(x < 1.98, 1.0, np.nan)
def g_right(x):
    x=np.asarray(x); return np.where(x > 2.02, 1.0, np.nan)
fig, ax = plt.subplots(figsize=(5.2,4.0))
make_standard_graph(ax,[
    {'expr':g_left,'deriv':lambda x:np.zeros_like(x,dtype=float),'color':'steelblue','label':None},
    {'expr':g_right,'deriv':lambda x:np.zeros_like(x,dtype=float),'color':'steelblue','label':None},
])
ax.plot(2,1,'o',markerfacecolor='white',markeredgecolor='steelblue',markeredgewidth=1.8,markersize=7)
ax.plot(2,0,'o',color='steelblue',markersize=7)
_save(fig,'g_limit_point.png')

fig, ax = plt.subplots(figsize=(5.0,5.0))
_draw_unit_circle_base(ax, show_angle_lines=False)
theta=.72
P=(np.cos(theta),np.sin(theta)); O=(0,0); A=(1,0); T=(1,np.tan(theta)); Q=(P[0],0)
# First-quadrant arc emphasized for the proof.
th=np.linspace(0,np.pi/2,300)
ax.plot(np.cos(th),np.sin(th),color='steelblue',linewidth=1.5,zorder=4)
for p,q in [(O,P),(O,A),(A,T),(O,T),(P,Q)]:
    ax.plot([p[0],q[0]],[p[1],q[1]],color='#222222',linewidth=1.0,zorder=5)
ax.plot(P[0],P[1],'o',color='steelblue',markersize=5,zorder=6); ax.plot(1,0,'o',color='#222222',markersize=4,zorder=6)
arc=np.linspace(0,theta,60); ax.plot(.18*np.cos(arc),.18*np.sin(arc),color='firebrick',linewidth=1.2,zorder=6)
ax.text(.20,.08,r'$\theta$',fontsize=11); ax.text(-.04,-.05,'O',fontsize=10); ax.text(Q[0]-.02,-.07,'Q',fontsize=10)
ax.text(1.02,-.07,'A (1, 0)',fontsize=10); ax.text(P[0]+.03,P[1]+.03,'P',fontsize=10); ax.text(1.03,T[1]+.02,'T',fontsize=10)
_save(fig,'squeeze_unit_circle.png')
