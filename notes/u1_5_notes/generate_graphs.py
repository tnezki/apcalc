...
# === Unit 1 Section 1.5 Notes-specific figure ===
fig, ax = plt.subplots(figsize=(3.5, 3.5))
make_standard_graph(ax, [
    {'expr': lambda x: 0.08*x**3 - 0.5*x + 1, 'deriv': lambda x: 0.24*x**2 - 0.5, 'color': 'steelblue', 'label': r'$f$'},
    {'expr': lambda x: np.zeros_like(x) + 2.5, 'deriv': lambda x: np.zeros_like(x), 'color': 'firebrick', 'label': r'$y=N$'},
], title=r'Continuous graph and an intermediate target level')
save_graph(fig, 'u1_s5_ivt_model.png')
plt.close(fig)
