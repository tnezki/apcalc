...
# === Unit 1 Section 1.3 Notes-specific figure ===
def _u1_s3_mid(x):
    ...
    out[mask] = 5.0 * vals[mask] * np.sin(5.0 / vals[mask])
...
def _u1_s3_mid_d(x):
    ...
    out = np.zeros_like(vals)
    mask = np.abs(vals) > 1e-12
    out[mask] = 5.0 * np.sin(5.0 / vals[mask]) - 25.0 * np.cos(5.0 / vals[mask]) / vals[mask]
    return float(out[0]) if scalar else out
...
make_standard_graph(ax, [
    {'expr': lambda x: -5*np.abs(x), 'deriv': lambda x: -5*_u1_s3_abs_d(x), 'color': 'firebrick', 'label': r'$-5|x|$'},
    {'expr': _u1_s3_mid, 'deriv': _u1_s3_mid_d, 'color': 'darkorange', 'label': r'$5x\sin(5/x)$'},
    {'expr': lambda x: 5*np.abs(x), 'deriv': lambda x: 5*_u1_s3_abs_d(x), 'color': 'steelblue', 'label': r'$5|x|$'},
], title=r'An oscillating function squeezed near $x=0$')
save_graph(fig, 'u1_s3_squeeze_model.png')
plt.close(fig)
