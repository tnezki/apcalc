...
# === Unit 1 Section 1.4 Notes-specific figure ===
fig, ax = plt.subplots(figsize=(3.5, 3.5))
make_piecewise_graph(ax, [
    {'expr': lambda x: 0.5*x + 1, 'deriv': lambda x: np.zeros_like(x) + 0.5,
     'domain': (-10, 1), 'include_left': True, 'include_right': False,
     'arrow_left': True, 'arrow_right': False},
    {'expr': lambda x: 0.5*x + 3, 'deriv': lambda x: np.zeros_like(x) + 0.5,
     'domain': (1, 10), 'include_left': True, 'include_right': True,
     'arrow_left': False, 'arrow_right': True},
], title=r'A boundary with unequal one-sided limits')
save_graph(fig, 'u1_s4_jump_model.png')
plt.close(fig)
