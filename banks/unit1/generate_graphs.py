...
# === UNIT 1 CANONICAL BANK GENERATION BLOCKS (appended below authoritative marker) ===
def _save_pw(filename,pieces):
    fig,ax=plt.subplots(figsize=(4,4)); make_piecewise_graph(ax,pieces,title=''); fig.savefig(os.path.join(OUTPUT_DIR,filename),dpi=144,bbox_inches='tight'); plt.close(fig)
def _save_std(filename,functions):
    fig,ax=plt.subplots(figsize=(4,4)); make_standard_graph(ax,functions,title=''); fig.savefig(os.path.join(OUTPUT_DIR,filename),dpi=144,bbox_inches='tight'); plt.close(fig)

def C(v): return lambda x: np.full_like(np.asarray(x,dtype=float),float(v))
def Z(x): return np.zeros_like(np.asarray(x,dtype=float))

_save_pw('u1_s1_hole.png',[
 {'expr':lambda x:x+1,'deriv':lambda x:np.ones_like(x),'domain':(-10,2),'include_left':False,'include_right':False,'arrow_left':True},
 {'expr':lambda x:x+1,'deriv':lambda x:np.ones_like(x),'domain':(2,10),'include_left':False,'include_right':False,'arrow_right':True},
 {'expr':C(6),'deriv':Z,'domain':(2,2),'include_left':True,'include_right':True}])
_save_pw('u1_s2_jump.png',[
 {'expr':lambda x:-0.5*x+2,'deriv':lambda x:-0.5*np.ones_like(x),'domain':(-10,3),'include_left':False,'include_right':False,'arrow_left':True},
 {'expr':lambda x:x-2,'deriv':lambda x:np.ones_like(x),'domain':(3,10),'include_left':True,'include_right':False,'arrow_right':True}])
_save_std('u1_s3_squeeze.png',[
 {'expr':lambda x:x*x,'deriv':lambda x:2*x,'color':'steelblue','label':r'$x^2$'},
 {'expr':lambda x:-x*x,'deriv':lambda x:-2*x,'color':'firebrick','label':r'$-x^2$'},
 {'expr':lambda x:x*x*np.sin(1/np.where(np.abs(x)<1e-9,1e-9,x)),'deriv':lambda x:2*x*np.sin(1/np.where(np.abs(x)<1e-9,1e-9,x))-np.cos(1/np.where(np.abs(x)<1e-9,1e-9,x)),'color':'darkorange','label':r'$x^2\sin(1/x)$'}])
_save_pw('u1_s4_piecewise.png',[
 {'expr':lambda x:x+2,'deriv':lambda x:np.ones_like(x),'domain':(-10,1),'include_left':False,'include_right':True,'arrow_left':True},
 {'expr':lambda x:-x+4,'deriv':lambda x:-np.ones_like(x),'domain':(1,10),'include_left':True,'include_right':False,'arrow_right':True}])
_save_std('u1_s5_asymptote.png',[{'expr':lambda x:2+3/(x-1),'deriv':lambda x:-3/(x-1)**2,'color':'steelblue','label':None}])
_save_pw('u1_et_s2.png',[
 {'expr':lambda x:x+1,'deriv':lambda x:np.ones_like(x),'domain':(-10,-2),'include_left':False,'include_right':False,'arrow_left':True},
 {'expr':lambda x:-x+1,'deriv':lambda x:-np.ones_like(x),'domain':(-2,10),'include_left':True,'include_right':False,'arrow_right':True}])
_save_pw('u1_ext_multi.png',[
 {'expr':lambda x:0.5*x+3,'deriv':lambda x:0.5*np.ones_like(x),'domain':(-10,2),'include_left':False,'include_right':False,'arrow_left':True},
 {'expr':lambda x:-x+7,'deriv':lambda x:-np.ones_like(x),'domain':(2,10),'include_left':False,'include_right':False,'arrow_right':True},
 {'expr':C(1),'deriv':Z,'domain':(2,2),'include_left':True,'include_right':True}])
_save_pw('u1_perf1.png',[
 {'expr':lambda x:x+4,'deriv':lambda x:np.ones_like(x),'domain':(-10,2),'include_left':False,'include_right':False,'arrow_left':True},
 {'expr':lambda x:x+4,'deriv':lambda x:np.ones_like(x),'domain':(2,10),'include_left':False,'include_right':False,'arrow_right':True},
 {'expr':C(-1),'deriv':Z,'domain':(2,2),'include_left':True,'include_right':True}])
for fn,a,L,R,V in [('u1_sum_v1.png',2,4,4,7),('u1_sum_v2.png',-1,3,3,-2),('u1_sum_v3.png',3,-1,-1,5),('u1_sum_v4.png',1,6,6,0)]:
    _save_pw(fn,[
      {'expr':lambda x,A=a,Y=L:0*x+Y,'deriv':Z,'domain':(-10,a),'include_left':False,'include_right':False,'arrow_left':True},
      {'expr':lambda x,A=a,Y=R:0*x+Y,'deriv':Z,'domain':(a,10),'include_left':False,'include_right':False,'arrow_right':True},
      {'expr':C(V),'deriv':Z,'domain':(a,a),'include_left':True,'include_right':True}])
