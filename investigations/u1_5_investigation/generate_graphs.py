...
funcs=[
 {'expr':lambda x:8+3*x,'deriv':lambda x:np.full_like(x,3,dtype=float),'color':'steelblue','label':'Park A'},
 {'expr':lambda x:2+5*x,'deriv':lambda x:np.full_like(x,5,dtype=float),'color':'firebrick','label':'Park B'}]
make_context_graph(ax,funcs,0,8,0,45,xlabel='Rides',ylabel='Cost ($)',title='')
save_graph(fig,'park_cost_graph.png')
plt.close(fig)
