...
# === UNIT 1 INVESTIGATION FIGURES — APPENDED BUILD BLOCK ===

import matplotlib.pyplot as plt

def _std(filename, expr, deriv):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    make_standard_graph(ax,[{'expr':expr,'deriv':deriv,'color':'steelblue'}],title='')
    save_graph(fig,filename)
    plt.close(fig)

_std('card_linear.png', lambda x:2*x+1, lambda x:np.full_like(x,2,dtype=float))
_std('card_absolute.png', lambda x:np.abs(x), lambda x:np.sign(x))
_std('card_exponential.png', lambda x:np.power(2.0,x), lambda x:np.log(2.0)*np.power(2.0,x))
_std('card_quadratic.png', lambda x:x**2, lambda x:2*x)
