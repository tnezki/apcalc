...
def _context_line(filename, m, b, xmax, ymax):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    funcs=[{'expr':lambda x,m=m,b=b: m*x+b,
            'deriv':lambda x,m=m: np.full_like(x,m,dtype=float),
            'color':'steelblue'}]
    make_context_graph(ax, funcs, 0, xmax, 0, ymax, xlabel='Input', ylabel='Output', title='')
    save_graph(fig, filename)
    plt.close(fig)

_context_line('ticket_graph.png',4,12,5,35)
_context_line('phone_plan_graph.png',2,8,5,35)
_context_line('walkathon_graph.png',3,0,5,35)
_context_line('plant_growth_graph.png',4,6,4,24)
_context_line('mystery_graph.png',-2,16,6,20)
